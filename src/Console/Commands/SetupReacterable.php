...
final class SetupReacterable extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (Schema::hasColumn($model->getTable(), 'love_reacter_id')) {
            $this->error(sprintf(
                'Column `love_reacter_id` already exists in `%s` database table.',
                $model->getTable()
            ));
    ...
    private function createMigrationForModel(Model $model): void
    {
        ...
        $column = 'love_reacter_id';
        $foreignTable = 'love_reacters';
